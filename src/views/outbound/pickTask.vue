...
// 状态配置
const { getLabel } = useStatusConfig("OUTBOUND_ORDER");

// 数据
const loading = ref(false);
const error = ref(false);
const order = ref<OutboundOrder | null>(null);
const items = ref<OutboundOrderItem[]>([]);
...
async function loadData() {
  ...
  try {
    const res = await getOutboundOrder(id);
...
async function handleCancel() {
  ...
  try {
    ...
    await cancelOutboundOrder(order.value.id);
...
async function handlePick() {
  ...
  try {
    ...
    await pickOutboundOrder(order.value.id);
...
async function handleReadyToShip() {
  ...
  try {
    ...
    await readyToShipOutboundOrder(order.value.id);
...
async function handleShip() {
  ...
  try {
    ...
    await shipOutboundOrder(order.value.id, { locationId: "" });
...
<template>
  <div class="page detail-page">
    ...
    <template v-else-if="order">
      ...
      <el-card shadow="never" class="section items-card">
        ...
        >
          ...
          <el-table-column prop="locationCode" label="来源库位" min-width="150">
            <template #default="{ row }">
              {{ row.location_code || "-" }}
            </template>
          </el-table-column>
    ...
    <div class="note">出库单详情页，展示订单基础信息与商品明细。</div>
  </div>
</template>
